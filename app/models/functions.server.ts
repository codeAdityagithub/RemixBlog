...
export async function deleteBlog(blogId: string, userId: string) {
  ...
  try {
    ...
    await Engagements.deleteMany({ blogId: deletedBlog._id }, { session });
    await Comments.deleteMany({ blogId: deletedBlog._id }, { session });
    await Replies.deleteMany({ blogId: deletedBlog._id }, { session });
